...
- � **Расчет освобождаемого места** - показывает сколько места можно освободить и сколько фактически освобождено
...
- 🌍 **Абсолютные пути** в отчетах и JSON независимо от способа запуска
...
- JSON файл с информацией о дубликатах (с **абсолютными путями** файлов), готовый для очистки
- Markdown отчёт сканирования (`-scan-report`), c группами дубликатов, активными ссылками и **расчетом потенциально освобождаемого места**
- Консольный вывод с информацией о найденных дубликатах и **общем объеме места, который можно освободить**
...
2. 📋 Один экземпляр каждого дубликата копируется в папку назначения (с **абсолютным путем**)
3. 🗑️ Все остальные дубликаты удаляются из исходных мест
4. 🔗 На месте удалённых файлов создаются символические ссылки
5. 💾 Подсчитывается **фактически освобожденное место** для каждой группы и в целом
6. 📊 Генерируется подробный отчет с **итоговой статистикой** (количество обработанных групп, созданных симлинков, освобожденного места)
